...
export default function LibraryPage() {
  ...
  const handleTrendingCardClick = (asset: any) => {
    setSelectedTrendingAsset(
      asset._id === selectedTrendingAsset?._id ? null : asset
    );
  ...
  return (
    <div className="max-w-5xl px-48 mx-auto bg-gray-200 p-6 rounded-lg shadow-md">
      ...
      <h1 className="text-sm mb-5 w-full text-gray-600 ">
        Created top picks from this week
      </h1>
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          {selectedKPI.map((kpi: any) => (
            <div key={kpi.id} className="mb-4 border p-3 rounded-lg">
              ...
              <p>{kpi.description}</p>
              <p>
                <strong>Metric ID:</strong> {kpi.metricID}
              </p>
              <p>
                <strong>Calculation:</strong> {kpi.calculation}
              </p>
              <p>
                <strong>Visuals:</strong> {kpi.visualsAvailable.join(", ")}
              </p>
            </div>
          ))}
        </div>
      ...
      <h6 className="text-sm mb-4 w-full text-gray-600">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-4">
        {trendAssets.map((trendAsset) => (
          <AssetCard
            ...
            isSelected={
              selectedTrendingAsset &&
              selectedTrendingAsset._id === trendAsset._id
            }
          />
        ))}
      </div>
